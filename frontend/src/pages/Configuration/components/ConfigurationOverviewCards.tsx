              ...
              <Box display="flex" alignItems="center" gap={2}>
                ...
                <Box>
                  <Typography variant="h6" fontWeight={600}>
                    {healthStatus?.status === 'ok' ? 'Backend online' : 'Backend needs attention'}
                  </Typography>
                  <Box display="flex" alignItems="center" gap={1} mt={0.5}>
                    ...
                    <Chip
                      label={healthStatus?.status === 'ok' ? 'Running' : 'Error'}
                      size="small"
        ...
        <Card>
          ...
          <CardContent>
            <Box display="flex" alignItems="center" gap={2}>
              ...
              <Box>
                <Typography variant="body2" color="text.secondary">
                  Mode code: {usbMode?.current_mode ?? 'N/A'}
                </Typography>
                {usbMode?.temporary_mode && (
                  <Typography variant="caption" color="warning.main" display="block">
                    Pending temporary profile: {getModeNameByValue(usbMode.temporary_mode)}
                  </Typography>
                )}
                {usbMode?.permanent_mode && (
                  <Typography variant="caption" color="text.secondary" display="block">
                    Saved permanent profile: {getModeNameByValue(usbMode.permanent_mode)}
                  </Typography>
                )}
              </Box>
            </Box>
